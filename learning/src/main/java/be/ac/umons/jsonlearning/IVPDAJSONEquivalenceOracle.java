...
public interface IVPDAJSONEquivalenceOracle extends EquivalenceOracle<OneSEVPA<?, JSONSymbol>, JSONSymbol, Boolean> {
    ...
    default <L> CounterexampleWithRelations<L> counterexampleAndRelationFromKeyGraph(OneSEVPA<L, JSONSymbol> hypo) {
        ...
        final WitnessRelation<L> witnessRelation = WitnessRelation.computeWitnessRelation(hypo, reachabilityRelation);
        final KeyGraph<L> keyGraph = new KeyGraph<>(hypo, reachabilityRelation, witnessRelation);
        if (keyGraph == null || keyGraph.isValid()) {
            return null;
        }
        assert keyGraph.getWitnessCycle() != null;
    ...
    default <L1, L2> CounterexampleWithRelations<L2> counterexampleAndRelationFromKeyGraph(OneSEVPA<L1, JSONSymbol> previousHypothesis, ReachabilityRelation<L1> previousReachabilityRelation, WitnessRelation<L1> previousWitnessRelation, OneSEVPA<L2, JSONSymbol> currentHypothesis) {
        ...
        final WitnessRelation<L2> witnessRelation = WitnessRelation.computeWitnessRelation(previousHypothesis, previousWitnessRelation, currentHypothesis, reachabilityRelation);

        final KeyGraph<L2> keyGraph = new KeyGraph<>(currentHypothesis, reachabilityRelation, witnessRelation);
        if (keyGraph == null || keyGraph.isValid()) {
            return null;
        }
        assert keyGraph.getWitnessCycle() != null;
